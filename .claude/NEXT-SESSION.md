...
- **Committed on the branch:** the extension, `pilot-webr.qmd` (throwaway pilot — NOT in the book nav / `quarto inspect` inputs, so CI ignores it), `tools/webr-verify/`, ch.2 + `_quarto.yml` + `render.yml`, `.gitignore`. All on **PR #25**.
- **TODO next (Phase-2 rollout):** (1) optional belt-and-suspenders: a Playwright in-browser click-through of ch.2's live cells (both halves already proven independently); (2) **roll out to more chapters** the same way (convert self-contained chunks; keep state-coupled ones static); (3) migrate the shinyapps.io exercises → in-page `{webr}` exercises (the "For you to do" links); (4) bump book `renv.lock` itssl pin to `v0.2.0` once a chapter uses the potato data (ch.2 doesn't, so still `v0.1.0`). Pattern is proven — rollout is incremental.
