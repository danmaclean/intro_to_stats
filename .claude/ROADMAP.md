...
- [→] **Migrate the shinyapps "For you to do" exercises into the page (IN PROGRESS — current focus).** Source = **`TeamMacLean/shinyapps`** (private; cloned to `~/Desktop/shinyapps`); learnr `.Rmd` tutorials. Mechanism decided: **code exercises → quarto-live `{webr}` exercises** (gradethis `grade_this_code()`); **MCQs → `naquiz` + a "Why?" collapse callout** (naquiz has no per-answer feedback); ch2 sliders deferred. **CH3 (`ttests`) PILOT DONE** — 3 graded penguins exercises + 6 MCQs + Why reveals; renders clean. Remaining six: `linear_models_background` (ch1), `linear_models` (ch2, +4 sliders), `anova` (ch4), `type` (ch5), `chisq` (ch6), `r-start` (r-fundamentals; source also in `itssl` history `c9b071f:inst/tutorials/r-start/r-start.Rmd`). Keep shinyapps live until parity.
