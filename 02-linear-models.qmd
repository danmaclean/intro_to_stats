...
[^1]: Here we're using the example data, but this is never different whether you're using example or 'real' data. The maths is the same and the code is the same.
...
:::{.callout-tip}
## For you to do
Use the interactive sliders and questions below to check your understanding — everything runs **in your browser**. Drag a slider and watch the plot update, then answer. (The original standalone quiz is still [available online](https://tsl-bioinformatics.shinyapps.io/linear_models/).)
:::
...
```{ojs}
//| echo: false
viewof aval = Inputs.range([-5, 5], {value: 1, step: 0.25, label: "a (coefficient of x)"})
```
...
#| fig-height: 5
library(itssl)
its_axplusb_time(xrange = c(-5, 5), a = aval, b = 0, yrange = c(-5, 5))
```
...
```{ojs}
//| echo: false
viewof bval = Inputs.range([-5, 5], {value: 1, step: 0.25, label: "b (intercept)"})
```
...
#| fig-height: 5
library(itssl)
its_axplusb_time(xrange = c(-5, 5), a = 1, b = bval, yrange = c(-5, 5))
```
...
```{ojs}
//| echo: false
viewof coefval = Inputs.range([-10, 10], {value: 2, step: 0.5, label: "Coefficient (a) of x"})
```

```{ojs}
//| echo: false
viewof noiseval = Inputs.range([0, 20], {value: 2, step: 0.5, label: "Amount of random noise (experimental variability)"})
```
...
library(itssl)
set.seed(123)
df <- tibble::tibble(x = runif(20, 5, 15), y = x * coefval + rnorm(20, 0, noiseval))
print(its_plot_xy_time(df, line = TRUE, residuals = TRUE))
model <- lm(y ~ x, data = df)
s <- summary(model)
data.frame(
  Statistic = c("adj.r.squared", "F", "p"),
  Value = round(c(s$adj.r.squared,
                  s$fstatistic[[1]],
                  pf(s$fstatistic[[1]], s$fstatistic[[2]], s$fstatistic[[3]], lower.tail = FALSE)), 4)
)
```
