...
:::{.callout-tip}
## For you to do
Work through the tutorial below — the code runs **in your browser**, no installation needed. Type your answer and press *Run*; use *Show solution* if you get stuck. (The original standalone version is still [available online](https://tsl-bioinformatics.shinyapps.io/ttests/).)
:::
...
```{webr}
#| setup: true
#| exercise: [ttest-str, ttest-summary, ttest-lm]
library(dplyr)
...
gradethis::grade_this_code(
  correct = "Nicely done — that's exactly it.",
  incorrect = "Remember to group by the right column, and to remove NA values (na.rm = TRUE) in your summary functions. If you'd like a more specific pointer: {code_feedback()}"
)
```
...
Recall that the $F$ statistic indicates the strength of the relationship between $x$ and $y$ — the size of, and fit to, the slope — which together indicate a real difference between the groups.
