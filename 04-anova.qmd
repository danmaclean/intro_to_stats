...
:::{.callout-tip}
## For you to do
Work through the tutorial below — the code runs **in your browser**, no installation needed. Type your answer and press *Run*; use *Show solution* if you get stuck. (The original standalone version is still [available online](https://tsl-bioinformatics.shinyapps.io/anova/).)
:::
...
```{webr}
#| setup: true
#| exercise: [anova-penguins-x, anova-penguins-1, anova-penguins-3, anova-interactions-1, anova-interactions-2, anova-interactions-3, anova-interactions-5, anova-interactions-7, anova-interactions-10]
...
set.seed("23456")
n <- 3
plant_weight <- bind_rows(
  data.frame(compost = "john_innes_1", supplement = "formula1", dry_weight = rnorm(n, 45, 10)),
  data.frame(compost = "john_innes_1", supplement = "formula2", dry_weight = rnorm(n, 30, 8)),
  data.frame(compost = "john_innes_2", supplement = "formula1", dry_weight = rnorm(n, 30, 8)),
  data.frame(compost = "john_innes_2", supplement = "formula2", dry_weight = rnorm(n, 45, 8))
)
plant_weight$compost    <- factor(plant_weight$compost,    levels = c("john_innes_2", "john_innes_1"))
plant_weight$supplement <- factor(plant_weight$supplement, levels = c("formula1", "formula2"))
```

### A multi-variable ANOVA

Inspect the `penguins` data.

```{webr}
#| exercise: anova-penguins-x
summary(...)
```
...
:::::{.question}
Which group are the response (measured outcome) variables in `penguins`?
...
:::::{.question}
From the model summary, which variables are having a significant effect on body mass?
...
:::::{.question}
Does the pattern suggest an interaction?
...
From the summary we can see a significant interaction. But the main effects also appear significant. This is confusing — but recall that the main effects can't be used if the interaction is significant.
...
```{webr}
#| exercise: anova-interactions-10
library(dplyr)
