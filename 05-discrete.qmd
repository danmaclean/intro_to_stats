...
:::{.callout-tip}
## For you to do
Work through the tutorial below — the code runs **in your browser**, no installation needed. Type your answer and press *Run*; use *Show solution* if you get stuck. (The original standalone version is still [available online](https://tsl-bioinformatics.shinyapps.io/type/).)
:::

```{webr}
#| setup: true
#| exercise: [type-0, type-3]
library(dplyr)
plant_greenness <- transmute(PlantGrowth, green_score = round(weight, digits = 0), group)
```

```{webr}
#| setup: true
#| exercise: [type-4, type-6, type-7]
library(dplyr)
plant_greenness <- transmute(PlantGrowth, green_score = as.factor(round(weight, digits = 0)), group)
ranked_plant_greenness <- mutate(plant_greenness, ranked_green_score = rank(green_score))
```

### Setting data type

Inspect the data frame `plant_greenness`, which scores how green a plant's leaves looked after a treatment — a by-eye category, labelled with a digit.
...
## Why?
The column `green_score` is set as *numeric* — but these scores are categories that just happen to be labelled with digits.
:::

:::::{.question}
What should the type for `green_score` be set to?
...
:::{.choice}
neither, that doesn't apply
:::
::::
:::::

Set the column types correctly. Use `transmute` from `dplyr` and the `factor` function, explicitly setting the factor levels as `1,2,3,4,5` and `6`.
...
plant_greenness <- transmute(plant_greenness,
                       green_score =
                               factor(green_score, levels = c("1","2","3","4","5","6"), ordered = TRUE),
                       group)
str(plant_greenness)
```
...
With the types set correctly we can plot the data meaningfully — a 'balloon' plot. Use `ggplot` and `geom_count()` to plot `green_score` by `group`.
...
:::{.choice .correct-choice}
The number of observations of each score
:::
::::
:::::
...
:::{.callout-note collapse="true"}
## Why?
In other contexts `n` is used for the sample size, but here it's just the number of observations of that score.
:::

### Linear models on ranked category data

Now make the model and do the hypothesis test with an ANOVA on a *ranked* version of `green_score`. Use `dplyr`'s `mutate` to add the ranks to a new data frame, and view it.
...
Now build the linear model — ranks as the response, group as the explanatory — and run Tukey's *post hoc* test of the hypothesis that there's no difference in mean rank between the groups.
...
:::::{.question}
At a pre-decided 0.05 $p$-value level, which is a correct interpretation of the hypothesis test?

::::{.choices}
:::{.choice}
trt1, ctrl and trt2 are all the same.
:::

:::{.choice}
trt2 is significantly higher than trt1.
:::

:::{.choice .correct-choice}
trt2 and trt1 are not likely to have the same mean rank, according to the Null Model.
:::

:::{.choice}
trt2 has a more significant mean rank than trt1 according to the Null Model.
:::
::::
:::::
