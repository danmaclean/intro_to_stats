...
:::{.callout-tip}
## For you to do
Work through the tutorial below — the code runs **in your browser**, no installation needed. Type your answer and press *Run*; use *Show solution* if you get stuck. (The original standalone version is still [available online](https://tsl-bioinformatics.shinyapps.io/chisq/).)
:::

When both the explanatory ($x$) and response ($y$) variables are non-continuous categories, we reach for the $\chi^2$ test instead of a linear model. Here we'll use a table of the *adult passengers* on the ill-fated ship Titanic, broken down by travel `Class` and `Sex`, in a data frame called `titanic`, and ask whether the split between the sexes differs from one class to another.

```{webr}
#| setup: true
#| exercise: [chi-0, chi-2, chi-3]
library(dplyr)
...
:::::{.question}
Which variable holds the counts (the number of passengers in each `Class` × `Sex` group)?
...
To use the data in a $\chi^2$ test we first tabulate it, with `xtabs` and an R formula. Put the variable we want to compare — `Class` — in the **rows** of the table. (Hint: the order of the explanatory variables in the formula decides which goes in the rows; change the order and see what happens.)
...
Now compare the classes pairwise. Base R's `pairwise.prop.test()` tests, for each pair of classes, whether the proportion of the sexes differs — and applies a multiple-testing (FDR) correction so we don't get fooled by doing many comparisons at once.

```{webr}
#| exercise: chi-3
pairwise.prop.test(..., p.adjust.method = "fdr")
```

```{webr}
#| exercise: chi-3
#| solution: true
pairwise.prop.test(tab_titanic, p.adjust.method = "fdr")
```
