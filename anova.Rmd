...
A better alternative (in the sense of flexibility for the user) is the `multcomp` package function `glht()` (general linear model hypothesis test), which is more flexible with respect to which designs and contrasts you can get out, at the expense of being a little more complicated. The basic case is straightforward though.
...
Working with multiple variables is complicated over the single case as we have the possibility of an interaction between the variables to consider. That is to say that the response in an experiment may be stronger under conditions `a and b` than in `a` or `b` alone even if `a` or `b` alone is greater than `c`. We can visualise this as straight lines like in the diagrams below
...
knitr::kable(dplyr::sample_n(food, 5))                                             
```
...
```{r,message=FALSE}
its_hot_dog_and_ice_cream_plot_time()
```

Without rushing ahead to do the modelling, we see that chocolate sauce has a slightly greater effect on enjoyment than mustard, whereas the enjoyment of the two food types is the same. So without a hypothesis test the dangerously thin conclusion would be 'use chocolate sauce to enhance your enjoyment of Hot Dogs or Ice Cream'.  We might be suspect of this conclusion not only because it seems not to line up with our intuitive sense of what's going on, but also because of those strange splits in the data. Look within each column and you can see that there is a definite clustering that is not a good sign, we possibly don't have a good specification of our data here.
...
```{r, message=FALSE}
its_hot_dog_and_ice_cream_two_ways_time()
...
For the two main non-interaction effects we could think to look at these as we've done before, using the `mcp()` function with the model with the explicit interaction in it, like this
...
knitr::kable(dplyr::sample_n(food_2, 6), align = "c")
```
...
Recall that when we introduced linear models we looked at the statistics of the coefficients (the column $Pr(>|t|)$ in the coeffecient block of the summary) _and_ the statistics of the whole model, the $p$-value at the end.
