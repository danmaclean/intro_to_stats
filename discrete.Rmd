...
A common place where these analyses goes wrong is when the analyst confuses the categoric with the continuous, and this is often caused by a bad naming scheme, as much as a lack of appreciation of the real nature of the data. Often using numbers as the names for categories can lead an unthinking analyst to make such a critical mistake. Let's consider a plant infection assay, often called an HR assay. Here a large-ish leaf of a plant is infected at points on its surface with either different strains or varieties of some pathogen and controls. After a period of incubation the severity of disease at each patch is assessed by eye and a score of disease applied. Sometimes that score will be a numeric one that may like this:
...
```{r}
library(ggplot2)
scores %>% ggplot() + aes(strain, score) + geom_jitter(aes(colour = replicate))
```
...
By using numbers for our score, then we have been able to use `geom_jitter()` data and it has generated additions to the scores that leave it with decimal points and that isn't correct either. It's more defensible on the grounds that if we didn't use `geom_jitter()` all the points would be in one place but there are alternatives to this that avoid misleading us mentally about what scores are.
...
```{r}
scores %>% ggplot() + aes(strain, score) + geom_boxplot() + geom_jitter(aes(colour = replicate))
```

What the cuss? This is a dreadful misrepresentation of the data! To work out why examine the mental contortions we're going through to read this. We see things that aren't really there, statistics like means and ranges, and relative magnitudes between categories. We think we can see these because the boxplot shows them. But the things like the mean of the data, its interquartile range etc are each a numeric feature calculated using formulae based on numbers. But our data are not numbers - they are category names that happen to have digits to represent them. We've squeezed numbers in where no numbers should be and created a problem of mental overloading - believing that because we've used numbers for categories we can treat them as numbers. On top of this the presentation tricks our mind into considering the results as if these statistics were real. 
...
The use of boxplots to show categories is seductive, it gives an impression of transparency and apparent differences when we use numbers, but it is misleading. It is misleading because the differences along the scale are not linear and the numbers don't behave as numbers. With these plots as a base an analysts next run statistical analysis in the way we have learned and generate results that area essentially meaningless. By not considering the categoric nature of our data explicitly we run the risk of misleading ourselves into making bad conclusions about data and eventually doing bad science. 

So we must avoid these confusions. First we must do this when we load our data into R, we'll need to make sure that we use the right data types. As a result of this we'll find that the plots we try to make will come out more appropriately - R will adapt to the data as far as it can. Second, when it comes to doing hypothesis tests we will have to use the proper test for the data type. As we are using the linear model as our basis for our interpretations we'll need to learn the adaptations to that. 
...
Base R provides the `read.csv()`  _write dot csv_ function for us to load `.csv` files and no option for loading Excel files. The `tidyverse` packages `readr` and `readxl` provide the `read_csv()` and `read_excel()` options respectively. You'll see `read.csv()` a lot in older code and quick tutorials, but it is a bit clunky. Some of `read.csv()`s default behaviours can be a bit unpredictable and we have to undo some of its work sometimes. So we'll look at the more consistent tidyverse offerings.

### Setting type with `read_csv()`

On loading data with this, we get a column specification - this tells us what R thinks each column contained, check it carefully to make sure R understands the data as you do. Here we'll load in the mock HR data we used above.
...
OK, weird. The boxplot clearly fails, there isn't the proper data to draw a boxplot with so coding our data as factors has saved us this error too. Though not explictly! At least it sends some sort of signal that something isn't right with our plot. 
...
## Linear models and a categoric response variable

How can we now apply our knowledge of linear models to find differences between $x$ categories when our $y$ is also categoric.

One thing we haven't considered up to now with our categories is that, although categories don't behave like numbers, they can behave like a queue, they often do have an intrinsic and meaningful order. A category like our HR score definitely has an order: $score\ 1 < score\ 2 < score\ 3 < score\ 4$ even if the intervals are not smooth or equal. Distinctly some categories don't have an order, e.g species 1 and species 2 aren't greater or lesser than each other. We need to treat ordered and unordered categories differently.
...
All we've done here is create some data in `observations` then turn it into a factor, the `levels` option sets the allowed levels and the `ordered` option tells R to take the given order. Inspecting the factor object by printing
